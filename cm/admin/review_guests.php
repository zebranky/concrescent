...
if (isset($_POST['start_id'])) {
	...
	$results = mysql_query(
		('SELECT * FROM '.db_table_name('guests').
		' WHERE `id` >= '.$start_id.
		' AND (`replaced_by` IS NULL OR `replaced_by` = 0)'.
		' ORDER BY `id`'.
		' LIMIT '.$batch_size),
		$conn
	);
	while ($result = mysql_fetch_assoc($results)) {
		...
		$html_content = render_list_row(
			...
			/* reorderable = */ false,
			/*        edit = */ false,
			/*      review = */ ('review_guest.php?id='.$result['id'])
		);
		$guests[] = array(
			'id' => $result['id'],
...
render_admin_head('Review Guest Applications');
...
?><script type="text/javascript">listPage({
	ajaxUrl: 'review_guests.php',
	entityType: 'guest applications',
	progressive: true,
	searchable: true,
});</script><?php

render_admin_body('Review Guest Applications');
...
render_list_table(array_merge(
	array('ID', 'Guest Name', 'Guest Type', '# Guests/Supporters'),
